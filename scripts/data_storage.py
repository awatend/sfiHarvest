...
class vehicle_nav(DynamicActor):
    def __init__(
        ...
        super().__init__(imc_id=9710)
        #self.target_driver = "manta-ntnu-1" #

        #self.target_brain = 0x3433
        #self.target_manta = 'manta-ntnu-1'
        self.target_list = ["ntnu-mr-usv","lauv-thor","ntnu-autonaut","ntnu-autonaut2","manta-ntnu-1","manta-ntnu-2","manta-ntnu-3", "lauv-simulator-1", "lauv-roald"]#, self.target_driver, self.target_brain, self.target_manta]
        ...
        self.state_array_lat_lon = np.empty((0, 4))  # [lat, lon, timestamp]
        self.coords_dict = {}
    ...
    def recv_Estate(self, msg: imcpy.EstimatedState):
        ...
        self.state_array_lat_lon = np.vstack((
            ...
            [self._lat, self._lon, self._depth,self._timestamp]
        ))
        ...
        name = name_map.get(self._id, self._id.title())

        vehicle_folder = os.path.join(self.output_dir, name)
        #print("folder",vehicle_folder)
        os.makedirs(vehicle_folder, exist_ok=True)
        current_time=datetime.now(timezone.utc)
        file_path = os.path.join(vehicle_folder, f"{self.nav_log_path}_{name}.csv")
        df = pd.DataFrame(
		            self.state_array_lat_lon,
		            columns=['lat', 'long', 'depth', 'timestamp']
		        )
